...
pub fn render_stacks_impl(f: &mut ratatui::Frame, app: &mut App, area: ratatui::layout::Rect) {
        ...
        app.stacks
            ...
            .map(|s| {
                ...
                let (upd_text, upd_style) =
                    image_update_indicator(app, resolve_stack_update_state(app, &s.name), bg);
                let mut state = String::new();
                let mut state_style = row_style;
                for c in app.containers.iter().filter(|c| {
                    stack_name_from_labels(&c.labels).as_deref() == Some(s.name.as_str())
                }) {
    ...
    state.select(Some(app.stacks_selected.min(rows.len().saturating_sub(1))));
    let table = Table::new(
        rows,
        [
            Constraint::Min(26),
