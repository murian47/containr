...
- Main list tables (containers/images/volumes/networks): `src/ui/render/tables.rs`
...
- Mode-specific key handling:
  ...
  - mode toggles/edit mode handling: `src/ui/input/modes.rs`
  - view navigation/focus/sidebar/list keys: `src/ui/input/views.rs`
...
- Command parser/executor: `src/ui/commands/cmdline_cmd.rs`
...
- Deployment orchestration actions: `src/ui/actions.rs`
- Lower-level deploy operations: `src/ui/core/ops.rs`
...
2. Wire alias/dispatch in `src/ui/commands/cmdline_cmd.rs` or `src/ui/commands/mod.rs`.
3. Update help text in `src/ui/render/help.rs`.
...
3. Verify dispatcher path in `src/ui/input/global.rs` or `src/ui/input/views.rs`.
4. Update help in `src/ui/render/help.rs`.

### Add/adjust a list column
1. Table structure/columns in `src/ui/render/tables.rs` (or view-specific renderer).
...
- Start from command text (`:...`): `src/ui/commands/cmdline_cmd.rs`
- Start from visual issue in a pane: `src/ui/render/shell.rs` -> related `src/ui/render/*`
- Start from template deploy issue: `src/ui/commands/templates_cmd.rs` -> `src/ui/actions.rs` -> `src/ui/core/ops.rs`
